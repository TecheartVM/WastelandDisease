package techeart.wastelanddisease.commands;
    ...
    private static class Actions
    {
        public static int set(CommandContext<CommandSourceStack> commandSourceStack) throws CommandSyntaxException
        {
            ...
            commandSourceStack.getSource().sendSuccess(
                    players.size() > 1 ?
                            new TranslatableComponent("command." + MainClass.MODID + ".rad_level.set.multiple", players.size(), value)
                            : new TranslatableComponent(
                                    "command." + MainClass.MODID + ".rad_level.set.single",
        ...
        public static int reset(CommandContext<CommandSourceStack> commandSourceStack) throws CommandSyntaxException
        {
            ...
            commandSourceStack.getSource().sendSuccess(
                    players.size() > 1 ?
                            new TranslatableComponent("command." + MainClass.MODID + ".rad_level.reset.multiple", players.size())
                            : new TranslatableComponent(
                                    "command." + MainClass.MODID + ".rad_level.reset.single",
                                    ((ServerPlayer)players.toArray()[0]).getDisplayName()
                            ),
                    true
            );
        ...
        public static int add(CommandContext<CommandSourceStack> commandSourceStack) throws CommandSyntaxException
        {
            ...
            commandSourceStack.getSource().sendSuccess(
                    players.size() > 1 ?
                            new TranslatableComponent(
                                    "command." + MainClass.MODID + ".rad_level.add.multiple",
                                    players.size(),
                                    value
                            )
                            : new TranslatableComponent(
                                    "command." + MainClass.MODID + ".rad_level.add.single",
                                    ((ServerPlayer)players.toArray()[0]).getDisplayName(),
                                    value
                            ),
                    true
        ...
        public static int get(CommandContext<CommandSourceStack> commandSourceStack) throws CommandSyntaxException
        {
            ...
            commandSourceStack.getSource().sendSuccess(
                    new TranslatableComponent("command." + MainClass.MODID + ".rad_level.get", player.getDisplayName(), value),
                    false
            );
            return 1;
        }
